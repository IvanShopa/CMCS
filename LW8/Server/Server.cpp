...
struct DrawPixel : Command {
  ...
  const uint16_t color;
  DrawPixel(const int16_t x0, const int16_t y0, const int16_t newX, const int16_t newY, const uint16_t color) : Command(DRAW_PIXEL_OPCODE), x0(x0), y0(y0), newX(newX), newY(newY), color(color) {};
};
struct DrawLine : Command {
  const int16_t x0, y0, x1, y1;
  const uint16_t color;
  DrawLine(const int16_t x0, const int16_t y0, const int16_t x1, const int16_t y1, const uint16_t color) : Command(DRAW_LINE_OPCODE), x0(x0), y0(y0), x1(x1), y1(y1), color(color) {}
};
struct DrawRectangle : Command {
  const int16_t x0, y0, x1, y1;
  const uint16_t color;
  DrawRectangle(const int16_t x0, const int16_t y0, const int16_t x1, const int16_t y1, const uint16_t color) : Command(DRAW_RECTANGLE_OPCODE), x0(x0), y0(y0), x1(x1), y1(y1), color(color) {}
};
struct FillRectangle : Command {
  const int16_t x0, y0, x1, y1;
  const uint16_t color;
  FillRectangle(const int16_t x0, const int16_t y0, const int16_t x1, const int16_t y1, const uint16_t color) : Command(FILL_RECTANGLE_OPCODE), x0(x0), y0(y0), x1(x1), y1(y1), color(color) {}
};
struct DrawEllipse : Command {
  const int16_t x0, y0, rx, ry;
  const uint16_t color;
  DrawEllipse(const int16_t x0, const int16_t y0, const int16_t rx, const int16_t ry, const uint16_t color) : Command(DRAW_ELLIPSE_OPCODE), x0(x0), y0(y0), rx(rx), ry(ry), color(color) {}
};
struct FillEllipse : Command {
  const int16_t x0, y0, rx, ry;
  const uint16_t color;
  FillEllipse(const int16_t x0, const int16_t y0, const int16_t rx, const int16_t ry, const uint16_t color) : Command(FILL_ELLIPSE_OPCODE), x0(x0), y0(y0), rx(rx), ry(ry), color(color) {}
};
struct Drawtext : Command {
  const int16_t x, y;
  const uint16_t color;
  const std::string text;
  Drawtext(int16_t x, int16_t y, uint16_t color, const std::string& text) : Command(DRAW_TEXT_OPCODE), x(x), y(y), color(color), text(text) {}
};
...
struct LoadSprite : Command {
  ...
  LoadSprite(uint16_t index, uint16_t width, uint16_t height, const std::vector<uint8_t>& data) : Command(LOAD_SPRITE_OPCODE), index(index), width(width), height(height), data(data) {}
};
struct ShowSprite : Command {
  const uint16_t index;
  const int16_t x;
  const int16_t y;
  ShowSprite(uint16_t index, int16_t x, int16_t y) : Command(SHOW_SPRITE_OPCODE), index(index), x(x), y(y) {}
};
int width = 800;
int height = 600;
int orientation = 0;
...
void DrawCommand(Command* command) {
  ...
  case DRAW_ELLIPSE_OPCODE: {
    ...
    SelectObject(hdc, pen);
    Ellipse(hdc, ellipseCommand->x0 - ellipseCommand->rx, ellipseCommand->y0 - ellipseCommand->ry, ellipseCommand->x0 + ellipseCommand->rx, ellipseCommand->y0 + ellipseCommand->ry);
    DeleteObject(pen);
    break;
  }
  case FILL_ELLIPSE_OPCODE: {
    ...
    SelectObject(hdc, brush);
    Ellipse(hdc, fillEllipseCommand->x0 - fillEllipseCommand->rx, fillEllipseCommand->y0 - fillEllipseCommand->ry, fillEllipseCommand->x0 + fillEllipseCommand->rx, fillEllipseCommand->y0 + fillEllipseCommand->ry);
    DeleteObject(brush);
    break;
  }
  case DRAW_TEXT_OPCODE: {
  ...
  case LOAD_SPRITE_OPCODE: {
    ...
    std::cout << "Sprite with index " << loadSpriteCommand->index << " loaded (" << loadSpriteCommand->width << "x" << loadSpriteCommand->height << ")." << std::endl;
    break;
  }
  case SHOW_SPRITE_OPCODE: {
    ...
    for (int row = 0; row < 16; ++row) {
      for (int col = 0; col < 16; ++col) {
        ...
        RECT rect = { showSpriteCommand->x + col * 10, showSpriteCommand->y + row * 10, showSpriteCommand->x + (col + 1) * 10, showSpriteCommand->y + (row + 1) * 10 };
        FillRect(hdc, &rect, brush);
        DeleteObject(brush);
      }
    }
    std::cout << "Sprite with index " << showSpriteCommand->index << " shown at (" << showSpriteCommand->x << ", " << showSpriteCommand->y << ")." << std::endl;
    break;
  }
  }
}
...
int main() {
  ...
  RegisterClass(&wc);
  hwnd = CreateWindow(wc.lpszClassName, L"", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 800, 600, NULL, NULL, wc.hInstance, NULL);
